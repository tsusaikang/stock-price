...
function refresh(key, ttl, fn) {
  ...
  const pending = fn()
    ...
    .catch((err) => {
      // 갱신에 실패했다고 멀쩡한 옛 값을 버리지는 않는다. 30초 뒤 다시 시도한다.
      if (prev && 'val' in prev) cache.set(key, { val: prev.val, exp: Date.now() + 30_000 });
      else cache.delete(key);
      throw err;
    });

  cache.set(key, { ...prev, pending });
  return pending;
}

/**
 * @param {object} [opts]
 * @param {boolean} [opts.swr] stale-while-revalidate. 만료돼도 옛 값을 즉시 돌려주고
 *   갱신은 뒤에서 한다. 해외 리전에서 네이버까지 왕복이 1~2초라, 이게 없으면 TTL이
 *   끝나는 순간 걸린 요청 하나가 수백 건의 재조회를 통째로 기다리게 된다.
 */
export function cached(key, ttl, fn, opts = {}) {
  const entry = cache.get(key);
  if (entry && entry.exp > Date.now()) return Promise.resolve(entry.val);

  const hasStale = entry && 'val' in entry;
  if (opts.swr && hasStale) {
    if (!entry.pending) refresh(key, ttl, fn).catch(() => {});
    return Promise.resolve(entry.val);
  }

  return entry?.pending ?? refresh(key, ttl, fn);
}
...
export const TTL = {
  quote: 60_000, // 시세·랭킹
  // 일봉은 하루에 한 번 확정된다. 장중 마지막 봉만 움직이는데 그건 시세로 따로 본다.
  // 짧게 잡으면 후보군 400여 종목이 동시에 만료돼 재조회 폭풍이 인다.
  daily: 1_800_000, // 30분
  search: 600_000,
};
...
// ── 업스트림 동시 호출 제한 ───────────────────────────────────────────
// SWR 갱신은 사용자를 기다리게 하지 않는 대신, 만료된 항목 수백 개가 한꺼번에
// 백그라운드 갱신에 들어갈 수 있다. pool()의 동시성은 즉시 반환 때문에 무력해지므로
// 네트워크 호출 자체를 여기서 막는다. 공개 URL로 열었을 때의 보호막이기도 하다.
...
function limited(fn) {
  ...
  return new Promise((resolve, reject) => {
    waiters.push(() => run(fn).then(resolve, reject));
  });
}
...
/** 지금 나가는 중인 upstream 호출 수 (진단용) */
export const upstreamLoad = () => ({ active, queued: waiters.length });

async function getJSON(url, headers = H_MOBILE) {
  const text = await limited(async () => {
